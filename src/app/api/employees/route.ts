...
export async function GET(req: NextRequest) {
  try {
    ...
    return success({ employees: result.rows }, "Berhasil mengambil data karyawan", 200);
  } catch {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return success({ employee: insertEmployeeResult.rows[0] }, "Karyawan berhasil didaftarkan", 201);
  } catch {
    await client.query("ROLLBACK");
    return error("Internal server error", 500);
  } finally {
